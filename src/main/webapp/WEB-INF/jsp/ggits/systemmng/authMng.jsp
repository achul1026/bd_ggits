        ...
        <div class="main_container">
            <aside class="snb_container">
                <div class="snb_wrap">
                    ...
                    <div class="side_txt">권한을 미리 설정하여 사용자별<br>권한을 지정할 수 있습니다.<br><br>각 권한을 클릭하여 수정하실 수 있습니다.</div>
                </div>
            </aside>
                    ...
                    <div class="flex-end">
                        <div class="btn_search_wrap float-none">
                        	<ul>
                        		...
                        		<li>
                        			<input type="text" id="searchContent" name="searchContent" placeholder="검색어를 입력하세요." class="input_same search_box" value="<c:out value='${searchContent}'/>">
                        		</li>
                        		<li>
                        			<button type="button" id="searchBtn" class="input_same search_box2" onclick="fnSearchList();">검색</button>
                        		</li>
                        		<li>
                        			<a href="${pageContext.request.contextPath}/system/auth/save.do" class="is-darkgreen-btn ">등록하기</a>
                        		</li>
                        	</ul>
                        </div>
                    </div>
                ...
                <table>
                    ...
                    <tr>
                  	...
                  	<c:forEach var="authList" items="${authList}">
	                    <tr onclick="detailView('<c:out value="${authList.authId}"/>')" class="pointer">
	                        <td><c:out value="${authList.rownum}"/></td>
	                        <td><c:out value="${authList.authNm}"/></td>
	                        <td><c:out value="${authList.descr}"/></td>
	                        <td><c:out value="${authList.grpCnt}"/></td>
	                        <fmt:formatDate var="updtDt" pattern="yyyy-MM-dd" value="${authList.updtDt}"/>
	                        <td><c:out value="${updtDt}"/></td>
	                        <fmt:formatDate var="crtDt" pattern="yyyy-MM-dd" value="${authList.crtDt}"/>
	                        <td><c:out value="${crtDt}"/></td>
	                    </tr>
                  	</c:forEach>
                  </table>
