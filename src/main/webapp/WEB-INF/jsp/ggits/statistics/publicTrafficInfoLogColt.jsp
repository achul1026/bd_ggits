	...
	<section class="main_section tab_set">
	    ...
	    <div class="group_btn_wrap tab_fc">
	        <div class="btn_search_wrap_left btn_search_wrap">
	        	<ul>
	        		<li>
	        			<button type="button" class="tab_btn_item is-dark-btn" onclick="startLoading(); location.href='${pageContext.request.contextPath}/statistics/data/recode/traffic_info_log_colt/list.do'">교통정보 이력집계</button>
	        		</li>
	        		<li>
	        			<button type="button" class="tab_btn_item is-dark-btn is-darkgreen-btn" onclick="startLoading(); location.href='${pageContext.request.contextPath}/statistics/data/recode/public_traffic_info_log_colt/list.do'">대중교통 정보 이력 집계</button>
	        		</li>
	        		<li>
		            ...
		            <div class="group2">
			            	...
			            	<div class="btn_search_wrap">
			            		<ul>
			            			...
			            			<li>
			            				노선번호 : <input type="text" class="input_same" id="searchContent" name="searchContent" placeholder="노선번호를 입력해주세요." value="<c:out value='${searchOption.searchContent}'/>">
			            			</li>
			            		</ul>
			            	</div>
	            ...
	            <table class="mt16">
	                <thead>
		                <tr>
		                    ...
		                    <th scope="col">구간</th>
		                    <th scope="col">이용량(명)</th>
		                    <th scope="col">단일환승(명)</th>
		                ...
		                <c:choose>
		                	<c:when test="${fn:length(logTotalList) > 0}">
		                		<c:forEach var="logInfo" items="${logTotalList}">
		                			<tr>
		                				<td class=""><c:out value='${logInfo.routeNm}'/></td>
		                				<td><c:out value='${logInfo.stStaNm}'/> ↔ <c:out value='${logInfo.edStaNm}'/></td>
		                				<td><fmt:formatNumber value="${logInfo.busUserCnt}" pattern="#,###" /></td>
		                				<td><fmt:formatNumber value="${logInfo.trsfrCnt}" pattern="#,###" /></td>
		                				<td>
		                					<fmt:parseDate value="${logInfo.clctYmd}" var="clctYmdDate" pattern="yyyyMMdd"/>
		                					<fmt:formatDate pattern="yyyy-MM-dd" value="${clctYmdDate}"/>
		                				</td>
		                			</tr>
		                		</c:forEach>
		                	</c:when>
		                	<c:otherwise>
		                		<tr>
		                			<td colspan="5">대중교통 이력 정보가 존재 하지 않습니다.</td>
		                		</tr>
		                	</c:otherwise>
		                </c:choose>
	                </tbody>
...
// 		}
	...
	function fnSearchList(){
		document.getElementById('searchForm').action= "${pageContext.request.contextPath}/statistics/data/recode/public_traffic_info_log_colt/list.do";
