	    ...
	    <div class="group_btn_wrap tab_fc">
	        <div class="btn_search_wrap_left btn_search_wrap">
	        	<ul>
	        		...
	        		<li>
	        			<button type="button" class="tab_btn_item is-dark-btn is-darkgreen-btn" onclick="startLoading(); location.href='${pageContext.request.contextPath}/statistics/traffic/analysis/public/bus_sttn_pasnr/list.do'">정류장별 승하차 승객</button>
	        		</li>
	        		<li>
		                	...
		                	<c:when test="${fn:length(statsList) > 0 }">
		                		<c:forEach var="statsInfo" items="${statsList}">
		                			<tr>
		                				...
		                				<td><c:out value='${statsInfo.sumPasng}'/></td>
		                				<td><c:out value='${statsInfo.rideUserCnt}'/> / <c:out value='${statsInfo.lndiUserCnt}'/> / <c:out value='${statsInfo.trnsitUserCnt}'/></td>
		                				<td><c:out value='${statsInfo.rideYmd}'/></td>		                				
		                			</tr>
		                		</c:forEach>
		                	</c:when>
		                	<c:otherwise>
		                		<tr>
		                			<td colspan="3">승하차 승객 정보가 존재하지 않습니다.</td>
		                		</tr>
		                	</c:otherwise>
		                </c:choose>
	                </tbody>
