	...
	<section class="main_section tab_set">
	    ...
	    <div class="group_btn_wrap tab_fc">
	        <div class="btn_search_wrap_left btn_search_wrap">
	        	<ul>
	        		<li>
	        			<button type="button" class="tab_btn_item is-dark-btn is-darkgreen-btn" onclick="startLoading(); location.href='${pageContext.request.contextPath}/statistics/traffic/analysis/facilities/traffic_facility_obtc_colt/list.do'">교통시설물 장애통계</button>
	        		</li>
	        		<li>
	        			<button type="button" class="tab_btn_item is-dark-btn" onclick="startLoading(); location.href='${pageContext.request.contextPath}/statistics/traffic/analysis/facilities/traffic_facility_equipment_log_info/list.do'">교통시설물 장비 로그 상세</button>
	        		</li>
	        	</ul>
	        </div>
	    </div>
		            ...
		            <div class="group2">
			            	...
			            	<div class="btn_search_wrap">
			            		<ul>
			            			...
			            			<li>
			            				검색 : <input type="text" class="input_same" name="searchContent" id="searchContent" placeholder="도로명/시설물 ID를 입력해주세요." value="<c:out value='${searchOption.searchContent}'/>">
			            			</li>
			            		</ul>
	            ...
	            <table class="mt16">
	                <thead>
		                <tr>
		                    ...
		                    <th scope="col">
	                            <select class="table-filter seltFilter" name="seltRoadRank" id="seltRoadRank">
		                            <option selected="selected"  value="all">도로유형</option>
		                            <c:forEach var="roadRank" items="${roadRankList}">
	                					<option value="<c:out value='${roadRank.cdId}'/>"><c:out value='${roadRank.cdNm}'/></option>
									</c:forEach>
		                        </select>   
		                    </th>
		                    <th scope="col">
	                            <select class="table-filter seltFilter" name="seltRoadDrct" id="seltRoadDrct">
		                            <option selected="selected" value="all">방향</option>
		                             <option>상행</option>
		                            <option>하행</option>
		                        </select>  
		                    </th>
		                    <th scope="col">
	                            <select class="table-filter seltFilter" name="seltFicltInfo" id="seltFicltInfo">
		                            <option selected="selected" value="all">시설물</option>
		                ...
		                <c:choose>
		                	<c:when test="${fn:length(trfFcltsSttsList) > 0}">
		                		<c:forEach var="trfFcltsStts" items="${trfFcltsSttsList}">
		                			<tr>
		                				<td class=""><c:out value='${trfFcltsStts.roadName}'/></td>
		                				<td><c:out value='${trfFcltsStts.roadGrd}'/></td>
		                				<td><c:out value='${trfFcltsStts.roadDrct}'/></td>
		                				<td><c:out value='${trfFcltsStts.fcltsType}'/></td>
		                				<td><c:out value='${trfFcltsStts.fcltsId}'/></td>
		                				<td><c:out value='${trfFcltsStts.trblOccurCnt}'/></td>
		                			</tr>
		                		</c:forEach>
		                	</c:when>
		                	<c:otherwise>
