	    ...
	    <div class="group_btn_wrap tab_fc">
	        <div class="btn_search_wrap_left btn_search_wrap">
	        	<ul>
	        		...
	        		<li>
	        			<button type="button" class="tab_btn_item is-dark-btn is-darkgreen-btn" onclick="startLoading(); location.href='${pageContext.request.contextPath}/statistics/traffic/analysis/public/bus_rout_pasnr/list.do'">노선구간별 승하차/재차 승객수</button>
	        		</li>
	        	</ul>
	        </div>
	    </div>
		            ...
		            <div class="group2">
						...
						<div class="btn_search_wrap">
							<ul>
								...
								<li>
									검색 : <input type="text" class="input_same" name="searchContent" id="searchContent" placeholder="구간명/노선번호를 입력해주세요." value="<c:out value='${searchOption.searchContent}'/>">
								</li>
							</ul>
		                ...
		                <c:choose>
		                	<c:when test="${fn:length(statsList) > 0}">
		                		<c:forEach var="statsInfo" items="${statsList}">
			                		<tr>
			                			<td class=""><c:out value='${statsInfo.stStaNm}'/> - <c:out value='${statsInfo.edStaNm}'/></td>
			                			<td><c:out value='${statsInfo.routeNm}'/></td>
			                			<td><c:out value='${statsInfo.totalPasngCnt}'/></td>
			                		</tr>
		                		</c:forEach>
		                	</c:when>
		                	<c:otherwise>
		                		<tr>
		                			<td colspan="4">노선 구간별 승하차/재차 승객수 정보가 존재하지 않습니다.</td>
		                		</tr>
		                	</c:otherwise>
		                </c:choose>
	                </tbody>
