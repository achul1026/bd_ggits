	...
	<section class="main_section tab_set">
	    ...
	    <div class="group_btn_wrap tab_fc">
	        <div class="table_btn_left">
	            ...
	            <button type="button" class="tab_btn_item is-dark-btn is-darkgreen-btn" onclick="location.href='${pageContext.request.contextPath}/statistics/traffic/analysis/bus_rout_stats/list.do'">시내버스 이동 현황통계</button>
			                ...
			                <c:choose>
			                	<c:when test="${fn:length(statsList) > 0 }">
			                		<c:forEach var="statsInfo" items="${statsList}">
			                			<tr>
			                				<td class="left">${statsInfo.busRouteId}</td>
			                				<td>${statsInfo.rideSttnNm}</td>
			                				<td>${statsInfo.lndiSttnNm}</td>
			                				<td>-</td>
			                				<td>-</td>
			                				<td>-</td>
			                				<td>${statsInfo.busUseCnt}</td>
			                			</tr>
			                		</c:forEach>
			                	</c:when>
			                	<c:otherwise>
			                		<tr>
			                			<td colspan="8">시내버스 이동현황 정보가 존재하지 않습니다.</td>
			                		</tr>
			                	</c:otherwise>
			                </c:choose>
<!-- 			                <tr> -->
