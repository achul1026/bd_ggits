	    ...
	    <div class="group_btn_wrap tab_fc">
	        <div class="btn_search_wrap_left btn_search_wrap">
	        	<ul>
	        		...
	        		<li>
	        			<button type="button" class="tab_btn_item is-dark-btn is-darkgreen-btn" onclick="startLoading(); location.href='${pageContext.request.contextPath}/statistics/traffic/analysis/bus_rout_stats/list.do'">시내버스 이동 현황통계</button>
	        		</li>
	        		<li>
			            ...
			            <div class="group2">
								...
								<div class="btn_search_wrap">
									<ul>
										...
										<li>
											검색 : <input type="text" class="input_same" name="searchContent" id="searchContent" placeholder="노선명을 입력해주세요.">
										</li>
									</ul>
			                ...
			                <c:choose>
			                	<c:when test="${fn:length(statsList) > 0 }">
			                		<c:forEach var="statsInfo" items="${statsList}">
			                			<tr>
			                				<td class=""><c:out value='${statsInfo.routeNm}'/></td>
			                				<td><c:out value='${statsInfo.stStaNm}'/></td>
			                				<td><c:out value='${statsInfo.edStaNm}'/></td>
			                				<td><c:out value='${statsInfo.mvmnDstne ne null ? statsInfo.mvmnDstne : "-"}'/></td>
			                			</tr>
			                		</c:forEach>
			                	</c:when>
			                	<c:otherwise>
			                		<tr>
			                			<td colspan="4">시내버스 이동현황 정보가 존재하지 않습니다.</td>
			                		</tr>
			                	</c:otherwise>
			                </c:choose>
		                </tbody>
...
// 		}
	...
	function fnSearchList(){
		document.getElementById('searchForm').action= "${pageContext.request.contextPath}/statistics/traffic/analysis/bus_rout_stats/list.do";
...
//     .setDataSetLabel('원활', '서행', '정체')
//     .setDataSet({
//             label: ['원활', '서행', '정체'],
//             data:[10, 10, 50],
//             backgroundColor: ['#7BDE4D', '#FFAF36',  '#FF4C4C'],
