...
<div class="modal-fillter-btn-wrap">
	  <c:out value='${gOpDataUseStatsRpt.dtlUrl}'/>
</div>
<div class="iframe-btn-wrap">
	<c:if test="${isUserChk and authCd eq 'AUC000'}">
		<button type="button"class="is-darkgreen-btn" onclick="deleteBtn('<c:out value="${gOpDataUseStatsRpt.rptId}"/>')">삭제</button>
	</c:if>
</div>
...
<script>
	function deleteBtn(rptId){
		new ModalBuilder().init().alertBoby("삭제하시면 다시 복구 할 수 없습니다.<br>삭제하시겠습니까?").footer(5,'삭제하기',function(button, modal){
			$.ajax({
				type : "get",
				url : "${pageContext.request.contextPath}/statistics/analysis/data/report/"+rptId+"/delete.ajax",
