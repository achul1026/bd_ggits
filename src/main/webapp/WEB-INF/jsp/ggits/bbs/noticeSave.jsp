...
<main>
    <div class="main_container">
        <aside class="snb_container">
            <div class="snb_wrap">
                <h3 class="side_title">공지사항 등록</h3>
                <div class="side_txt">공지사항을 작성해보세요.<br>사용자가 확인 할 수 있습니다.</div>
            </div>
        </aside>
        <section class="main_section">
            <h2 class="blind">공지사항 등록</h2>
            	<form id="noticeSaveFrm">
					<div class="contents_wrap mt24">
						<div class="group">
							<div class="group_text">공지사항 제목<span class="required-alert">*</span></div>
							<input type="text" id="ntcTitle" name="ntcTitle" class="input_same group_box" placeholder="제목을 입력해 주세요." maxlength="32" style="width: 40.5rem">
						</div>
						<div class="group">
							<div class="group_text">내용<span class="required-alert">*</span></div>
							<textarea id="ntcCnts" name="ntcCnts" class="textarea_same" placeholder="내용을 입력해주세요." style="display: block"></textarea>			
						</div>
		            </div>
           	...
           	<div class="group btn_search_wrap_left btn_search_wrap">
				<ul>
					<li>
						<button id="noticeSaveBtn" class="is-darkgreen-btn">저장</button>
...
<script>
	$("#cancelBtn").on('click',function(){
		new ModalBuilder().init().alertBoby("취소하시겠습니까? 작성중인 내용은 삭제 됩니다.").footer(5,'확인',function(button, modal){
		...
		$("#noticeSaveBtn").on('click',function(){
			if(isClick){
				...
				$.ajax({
					...
					dataType : "json",
					url : "${pageContext.request.contextPath}/bbs/notice/save.ajax",
					success : function(data) {
						if(data.code == 200){
							new ModalBuilder().init().successBody("공지사항 등록을 성공 하였습니다.").footer(4,'확인',function(button, modal){
								modal.close();
								location.href = ${pageContext.request.contextPath}"/bbs/notice/list.do";
								}).open();
							modalAlertWrap();
						} else {
							new ModalBuilder().init().alertBoby("공지사항 등록에 실패 하였습니다.").footer(4,'확인',function(button, modal){modal.close();}).open();
							modalAlertWrap();
							isClick = true;
						}
					}
