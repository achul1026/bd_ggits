			...
			<c:when test="${fn:length(list) > 0}">
				<c:forEach var="road" items="${list}" varStatus="status">
					<tr>
						...
						<td>
							[<c:out value='${road.adsiNm}'/> <c:out value='${road.adstdgNm}'/>]<br/>
							<c:out value='${road.roadName}'/>
						</td>
						<td>
							<fmt:formatNumber value="${road.avgSpeed}" pattern="#,###" />
							km/h
						</td>
					</tr>
				</c:forEach>
			...
			success : function(result) {
				...
				if(totalCnt > 0){
					$(result.data.list).each(function(index, item){
						html += '<tr>'+
								'<td>'+item.rnk+'</td>'+
								'<td>['+item.adsiNm+' '+item.adstdgNm+']<br/>'+item.roadName+'</td>'+
								'<td>'+numberComma(item.avgSpeed)+'km/h</td>'+
								'</tr>';
					})
				}else{
					html += '<tr>'+
