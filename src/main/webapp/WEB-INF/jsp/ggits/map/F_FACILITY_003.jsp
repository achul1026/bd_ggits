...
<div>
	...
	<div class="mt16">
	    <div class="dashboard_sub_title">교차로 번호를 찾기</div>
	    <input type="text" class="dashboard_input" id="mapCsrrdId" placeholder="교차로 번호를 입력해 주세요.">
	</div>
	...
	<div class="dashboard_node_box">
	    <div class="node_title">${paging.totalCount}개의 <span>"스마트 교차로"</span>를 찾았습니다.</div>
	    <div id="modalTbody">
		    <ul>
...
function fnSearchList(){
	...
	$.ajax({
		...
		url : "${pageContext.request.contextPath}/map/facility/information/F_FACILITY_003/data.ajax",
		success : function(result) {
			var html = '';
			var title = '';
			$(result.data.list).each(function(index, item){
				var detailHtml = 	'<ul>'+
										'<li class=\'popup_item\'>스마트 교차로명 : <span>'+item.name+'</span></li>'+
										'<li class=\'popup_item\'>경도 : <span>'+item.lon+'</span></li>'+
										'<li class=\'popup_item\'>위도 : <span>'+item.lat+'</span></li>'+
									'</ul>';
				html += '<ul>';
				html += 	'<li class="node_item">';
				html += 		'<button type="button" class="node_button_item" onclick="map.control.highlightingByLinkId(this,\''+item.nodeId+'\','+item.lon+','+item.lat+');" data-marker-html="'+detailHtml+'">['+item.name+']'+item.id+'</button>';
				html += 	'</li>';
				html +=	'</ul>';
	    	})
			...
			if(paging != null && paging != ''){
				title += paging.totalCount+'개의 <span>"스마트 교차로"</span>를 찾았습니다.';
