...
<div class="tab_bigbox_close">
    <div class="original_box clearfix">
    	<form id="searchForm">
    		...
    		<input type="hidden" id="searchResultType" name="searchResultType" value="">
	        ...
	        <div class="tab_item_box flex-center pt8 none" id="crossroadsListDiv">
				...
				<div class="gis_table_scroll">
					<div class="flex-center gap8">
					    <input type="text" class="bigdata_list_input" id="searchCrsrdNm" placeholder="차량번호를 입력해주세요.">
					    <input type="hidden" id="mapPage" name="page" value="1"/>
					    <button type="button" class="is-darkgreen-btn modal_input_srbtn tab_item_list_button" id="searchCrossBtn">찾기</button>
					</div>
					<div class="tab_item_box_list_box none">
						<table>
						    <colgroup>
						        <col style="width:10%">
						        <col style="width:10%">
...
// 	settingBigdataSearchParam("BD_EFFECT_ANALYSIS_003");
...
/* 		var searchForm = $("#searchForm").serialize();
	    		...
	    		<div class="dashboard_sub_container">
	   				<div class="dashboard_wrap dash_bg">
						<div class="tab_box_sub_header">
							<div class="tab_box_title">광역 긴급차량 운행정보</div>
							<div class="tab_box_close">
								<div class="opa_slider ui-slider ui-slider-horizontal ui-widget ui-widget-content ui-corner-all">
									...
									<span class="ui-slider-handle ui-state-default ui-corner-all" tabindex="0" style="left: 100%; background-color:#fff;"></span>
								</div>
		                ...
		                <div class="tab_item_box flex-center select_arrow">
		                	<div class="ftsize12">광역 긴급차량 운행 구간 내 인접 도로 및 교차로 소통 변화</div>
		                </div>
		                <div class="tab_item_box flex-center">
		                ...
		                <div class="tab_item_box">
		                    ...
		                    <div class="mt8 d3_chart">
		                    	<svg id="d3Chart"></svg>
		                    	<ul class="legend_box">
		                    		<li class="legend_item"><span></span>원활</li>
		                    		<li class="legend_item"><span></span>서행</li>
		                    		<li class="legend_item"><span></span>정체</li>
		                    	</ul>
		                    	<ul class="d3_label">
		                    		<li class="d3_label_item">정체</li>
		                    		<li class="d3_label_item">서행</li>
		                    		<li class="d3_label_item">원활</li>
		                    		<li class="d3_label_item">시간대</li>
		                    	</ul>
		                    </div>
		                </div>				
					</div>
        ...
        $('.tab_popup_close').on('click', function(){
        	$(this).closest('.dashboard_sub_container').remove();
        })
		...
		$(".opa_slider").slider({
  		...
  		var data = [
  			{ time: '11:10', value: 2100},
  			{ time: '11:15', value: 40},
  			{ time: '11:20', value: 700},
			{ time: '11:25', value: 2900},
			{ time: '11:30', value: 1900},
			{ time: '11:35', value: 10}
		]

  		const svg2 = d3.select("#d3Chart")
		...
		data.forEach((d, i) => {
			if (i < data.length - 1) {
				...
				.attr('stroke', function(d) {
				  return d[0].value <= 1000 ? '#7BDE4D' : d[0].value <= 2000 ? '#FFAF36' : d[0].value <= 3000 ? '#FF4C4C ' : '#ededed';
				})
				.attr('stroke-width', 6)
				.attr("x", 2)
				.attr('d', line)
				.on("mouseover", function(event, d) {
				  	tooltip.html("시간 : " + d[0].time + "<br/>소통량 : " + d[0].value)
