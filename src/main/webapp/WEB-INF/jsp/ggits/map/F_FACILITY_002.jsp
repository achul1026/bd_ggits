...
<div>
	...
	<div class="dashboard_node_box">
	    <div class="node_title">${paging.totalCount}개의 <span>"DSRC"</span>를 찾았습니다.</div>
	    <div id="modalTbody">
		    <ul>
		    	<c:forEach var="resultList" items="${resultList}">
					<li class="node_item">
						...
						<ul>
							<li class="popup_item">DSRC명 : <span>${resultList.name}</span></li>
							<li class="popup_item">설명 : <span>${resultList.descr}</span></li>
							<li class="popup_item">상태 : <span>${resultList.stts}</span></li>
	...
	$.ajax({
		...
		success : function(result) {
			...
			$(result.data.list).each(function(index, item){
				var detailHtml = 	'<ul>'+
										'<li class=\'popup_item\'>DSCR명 : <span>'+item.name+'</span></li>'+
										'<li class=\'popup_item\'>설명 : <span>'+item.descr+'</span></li>'+
										'<li class=\'popup_item\'>상태 : <span>'+item.stts+'</span></li>'+
			...
			if(paging != null && paging != ''){
				title += paging.totalCount+'개의 <span>"DSRC"</span>를 찾았습니다.';
