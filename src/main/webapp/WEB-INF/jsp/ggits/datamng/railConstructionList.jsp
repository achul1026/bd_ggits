    ...
    <main>
        <div class="main_container">
            <aside class="snb_container">
                <div class="snb_wrap">
                    <h3 class="side_title">철도 건설 자료 관리</h3>
                    <div class="side_txt">철도 건설 관련 자료를<br>등록 및 관리합니다.</div>
                </div>
            </aside>
            <section class="main_section">
                <h2 class="blind">철도 건설 자료 관리</h2>
								...
								<tr>
									...
									<td>
										<ul class="datamng_table_ul">
											<li>
												<a href="${pageContext.request.contextPath}/datamng/railConstruction/file/download.do?fileMngId=<c:out value='${fileMngTotInfo.fileMngId}'/>">
...
<script>
	...
	$(function(){
		...
		$('#uploadButton').on('click', function(){
		    new ModalBuilder().init('자료 업로드').ajaxBody("${pageContext.request.contextPath}/common/modal/datamng/rail/save.do").footer(1,'업로드',function(button, modal){
		    	if(isClick) {
		    		// 중복 클릭 방지
		    		isClick = false;
		    		...
		    		$.ajax({
		    			...
		    			dataType : "json",
		    			url : "${pageContext.request.contextPath}/datamng/railConstruction/save.ajax",
		    			success : function(result) {
		    				if(result.code == 200) {
		    					new ModalBuilder().init().successBody(result.message).footer(4, '확인', function(button, modal) {
		    						modal.close();
		    						location.href = ${pageContext.request.contextPath}"/datamng/railConstruction/list.do";
		    					}).open();
		    				} else {
	...
	function fnSearchList() {
		document.getElementById("searchForm").action = "${pageContext.request.contextPath}/datamng/railConstruction/list.do";
		...
		new ModalBuilder().init('자료 상세정보').ajaxBody("${pageContext.request.contextPath}/common/modal/datamng/road/update.do?fileMngId="+fileMngId).footer(1,'저장',function(button, modal){
			if(isClick){
		    	...
		    	$.ajax({
					...
					enctype : "multipart/form-data;charset=UTF-8",
					dataType : "json",
					url : "${pageContext.request.contextPath}/datamng/railConstruction/update.ajax",
	...
	function deleteFileBtn(fileMngId){
		new ModalBuilder().init().alertBoby("삭제하시면 다시 복구 할 수 없습니다.<br>삭제하시겠습니까?").footer(5,'삭제하기',function(button, modal){
			$.ajax({
				type : "get",
				dataType : "json",
				url : "${pageContext.request.contextPath}/datamng/railConstruction/"+fileMngId+"/delete.ajax",
				success : function(data) {
					if(data.code == 200){
						new ModalBuilder().init().successBody(data.message).footer(4,'확인',function(button, modal){
							modal.close();
